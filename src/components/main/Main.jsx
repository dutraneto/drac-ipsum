...
class Main extends Component {
  ...
  handleOptionChange = evt =>
    this.setState(
      ...
      () =>
        this.generateWords(
          this.state.selectedOption === 'words'
            ? this._words
            : this.state.selectedOption === 'sentences'
            ? this._sentences
            : this._sentences
        )
    )
  ...
  generateWords = stateOfWordsOrSentences => {
    ...
    for (let i = 1; i <= numberOfInputs; i++) {
      textToRender.push(
        stateOfWordsOrSentences[Math.floor(Math.random() * stateOfWordsOrSentences.length)]
      )
    }
    this.setState({ text: [...textToRender] })
  }
  ...
  generateOneParagraph = () => {
    ...
    for (let i = 0; i < 5; i++) {
      oneParagraph =
        oneParagraph + this._sentences[Math.floor(Math.random() * this._sentences.length)]
    }
    return oneParagraph
  }
  ...
  render() {
    ...
    const [paragraphs, sentences, words] = [
      selectedOption === 'paragraphs',
